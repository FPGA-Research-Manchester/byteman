...
inline void writeBitstreamMainSingleRegion(ofstream& fout, int slr, Rect2D writeRect)
{
	...
	for(int r = 0 ; r < sizeR ; r++){
		...
		if(selectedOptions.clb){
			...
			uint32_t farValue = XCAP_makeFAR(BLOCKTYPE_LOGIC, slr, srcR+r, fromCol, 0);
		...
		if(selectedOptions.bram){
			int framesToWrite = FRAMES_PER_BRAM_CONTENT_COLUMN * (numberOfBRAMsBeforeCol[toCol]-numberOfBRAMsBeforeCol[fromCol]);
			if(framesToWrite > 0) {
				uint32_t farValue = XCAP_makeFAR(BLOCKTYPE_BLOCKRAM, slr, srcR+r, numberOfBRAMsBeforeCol[fromCol], 0);
				XCAP_writeRegister(fout, XCAP::Register::FAR, farValue, loadedBitstreamEndianness);
				XCAP_writeCommand(fout, XCAP::Command::WCFG, loadedBitstreamEndianness);
				XCAP_writeNOP(fout, 1, 0, loadedBitstreamEndianness);
				XCAP_writeFDRI(fout, ((framesToWrite+1)*WORDS_PER_FRAME), loadedBitstreamEndianness);
				
				fout.write((char*)&bitstreamBRAM[srcR+r][numberOfBRAMsBeforeCol[fromCol]][0], framesToWrite * WORDS_PER_FRAME * 4);
				fout.write((char*)blankFrame, WORDS_PER_FRAME * 4);
			}
