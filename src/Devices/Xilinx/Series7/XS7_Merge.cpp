...
void XilinxSeries7::merge(XilinxSeries7* srcBitstream, string params, Rect2D src, Coord2D dst){
	...
	if(!selectedOptions.partialNotFull){
		...
		src.position = {0, 0};
		dst = {0, 0};
		src.size.row = numberOfRows * XS7_CLB_PER_CLOCK_REGION;
		src.size.col = maxNumberOfCols;
	}
	
	ensureRegionCompatibility(src, dst);
	
	if(src.position.row%XS7_CLB_PER_CLOCK_REGION != 0)
		throw runtime_error(string("Currently only full clock region height relocations are supported (use row numbers multiple of ").append(to_string(XS7_CLB_PER_CLOCK_REGION)).append(")."));
	if(src.size.row%XS7_CLB_PER_CLOCK_REGION != 0)
		throw runtime_error(string("Currently only full clock region height relocations are supported (use row numbers multiple of ").append(to_string(XS7_CLB_PER_CLOCK_REGION)).append(")."));
	if(dst.row%XS7_CLB_PER_CLOCK_REGION != 0)
		throw runtime_error(string("Currently only full clock region height relocations are supported (use row numbers multiple of ").append(to_string(XS7_CLB_PER_CLOCK_REGION)).append(")."));
