		...
		if(str::iff::stringContains(line, "SYNC") && (!str::iff::stringContains(line, "DESYNC"))){
			...
			if(str::iff::stringContains(line, "READ REG @")){
				...
				if(newRegAddr == XCAP::Register::UNDEFINED){
					...
					uint32_t farValue = XCAP_makeFAR(slr, b, r, c, m);
			...
			if(! synched){
				...
				if(instructionOPCODE == XCAP::Operation::NOP) {
					...
					if((regAddr == XCAP::Register::FDRI) && (wordCount > 0) && (wordCount % XUS_WORDS_PER_FRAME == 0)) {
						if(shadowFrameValid) {
							fout << dec << "# Shadow register contents are written to frame (BlockType="<<b<<", GlobalRowAddress="<<r<<", MajorAddress="<<c<<", MinorAddress="<<m<<") (Frame type: " << XilinxUltraScale::getFrameType(b,r,c) << ")."<<endl;
						...
						for(int i = 0 ; i < frameCount ; i++){
							...
							fout << dec << "Writing frame #" << i << " (BlockType="<<b<<", GlobalRowAddress="<<r<<", MajorAddress="<<c<<", MinorAddress="<<m<<") (Frame type: " << XilinxUltraScale::getFrameType(b,r,c) << ") hex data:"<<endl;
							uint32_t frameData[XUS_WORDS_PER_FRAME];
