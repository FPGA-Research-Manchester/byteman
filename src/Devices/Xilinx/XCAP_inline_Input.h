...
inline void parseBITheader(ifstream& fin, Endianness e)
{
	...
	for(int syncDetectionDone = 0 ; !syncDetectionDone ; ){
		...
		if(XCAP_makeSyncInstruction() == word)
			syncDetectionDone++;
		else
			fin.seekg(-3,ios::cur);
	}
...
inline Endianness parseBitstreamEndianness(ifstream& fin)
{
	...
	for(int syncDetectionDone = 0 ; !syncDetectionDone ; ){
		...
		if(XCAP_makeSyncInstruction() == word){
			...
			syncDetectionDone++;
		} else if(XCAP_makeSyncInstruction() == (Endian::NativeToBigEndian32(word))){
			returnVal = Endianness::BE;
			syncDetectionDone++;
		} else if(XCAP_makeSyncInstruction() == (Endian::NativeToLittleEndian32(word))){
			returnVal = Endianness::LE;
			syncDetectionDone++;
		} else if(XCAP_makeSyncInstruction() == Endian::BitSwap32(Endian::NativeToBigEndian32(word))){
			returnVal = Endianness::BE_BS;
			syncDetectionDone++;
		} else if(XCAP_makeSyncInstruction() == Endian::BitSwap32(Endian::NativeToLittleEndian32(word))){
				...
				for( ; wordCount > 0 ; ){
					if(regAddr == XCAP::Register::FAR){
						...
						if(shadowFrameValid){
							...
							} else if(b == BLOCKTYPE_BLOCKRAM) { // bram;
